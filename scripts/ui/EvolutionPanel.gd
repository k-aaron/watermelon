...
const FRUIT_SIZES: Array  = [20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70]
...
func _build() -> void:
	...
	for row_idx in range(ROWS.size()):
		...
		if row_idx < ROWS.size() - 1:
			...
			row_arrow.add_theme_font_size_override("font_size", 14)
...
func _make_fruit_cell(idx: int) -> Control:
	...
	# 아이콘 크기 (패널 표시용 축소)
	var display_radius: float = float(FRUIT_SIZES[idx]) / 2.0 * 0.55 + 9.0
	...
	score_lbl.add_theme_font_size_override("font_size", 9)
